...
const STEPS = [
  { status: 'Pending', label: 'Order Received',   sub: 'Your order is in the queue',           icon: '📋', color: 'purple' },
  { status: 'Cooking', label: 'Being Prepared',   sub: 'Our barista is working on it',         icon: '👨‍🍳', color: 'blue'   },
  { status: 'Ready',   label: 'Ready to Collect', sub: 'Your order is ready — enjoy!',         icon: '✅', color: 'green'  },
  { status: 'Served',  label: 'Enjoy!',           sub: 'Thank you for dining at Vesta Cafe',   icon: '🎉', color: 'gray'   },
];
    ...
    <div className={`bg-white rounded-2xl border-2 ${c.border} overflow-hidden shadow-sm animate-fade-in`}>
      <button onClick={() => setExpanded(v => !v)} className="w-full flex items-start justify-between p-4 text-left">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-2 mt-0.5">
          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${c.badge}`}>{order.status}</span>
      ...
      {expanded && (
        <div className="border-t border-gray-50 px-4 pt-3 pb-4 space-y-1.5 animate-fade-in">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">Items</span>
  ...
  return (
    <div className="mb-5 animate-fade-in">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="font-semibold text-gray-900 text-sm">Live Order Status</span>
        <span className="text-gray-400 text-xs">· updates in real-time</span>
      </div>
