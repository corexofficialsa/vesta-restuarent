    ...
    <header className="sticky top-0 z-50 glass border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <img src={logo} alt="Vesta" className="w-9 h-9 object-contain" />
          <div>
            <span className="font-bold text-gray-900 text-lg tracking-tight">Vesta</span>
            <span className="ml-1 text-purple-600 font-semibold text-lg tracking-tight">Restaurant</span>
          </div>
        </div>

        {/* Right side */}
        <div className="flex items-center gap-3">
          {/* Table badge — customer view only */}
          {activeTable && view === 'customer' && (
            <div className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 border border-purple-200 rounded-full">
              ...
              <span className="text-purple-700 text-sm font-medium">Table {activeTable}</span>
            </div>
          )}

          {/* Admin badge + logout — only visible when authenticated */}
          {isAuthenticated && view === 'admin' && (
            <div className="flex items-center gap-2">
              <span className="px-3 py-1.5 bg-purple-700 text-white text-xs font-semibold rounded-full">
                Kitchen
              </span>
              <button
                onClick={logout}
